...
`caffeine`, `bell`, `activewindow` (focused title), `tray` (click / right-click menu), `media` (now playing: click opens the
player card — spinning record with the cover art, seek, prev/play/next, shuffle/loop, player switcher; middle-click play/pause,
